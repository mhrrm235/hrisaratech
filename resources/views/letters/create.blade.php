                    ...
                    <form action="{{ route('letters.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="letter_type" name="letter_type" required>
                                ...
                                <option value="official" {{ old('letter_type') == 'official' ? 'selected' : '' }}>Official Letter</option>
                                <option value="memo" {{ old('letter_type') == 'memo' ? 'selected' : '' }}>Memorandum</option>
                                <option value="notice" {{ old('letter_type') == 'notice' ? 'selected' : '' }}>Notice</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="letter_template_id" name="letter_template_id">
                                ...
                                @foreach($templates as $template)
                                    <option value="{{ $template->id }}" {{ old('letter_template_id') == $template->id ? 'selected' : '' }}>{{ $template->name }}</option>
                                @endforeach
                            </select>
                        </div>
