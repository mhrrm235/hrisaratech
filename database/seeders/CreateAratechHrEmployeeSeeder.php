...
class CreateAratechHrEmployeeSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Create HR employee with new email if not exists
        $employeeEmail = 'hr@aratechnology.id';
        $employeeId = DB::table('employees')->where('email', $employeeEmail)->value('id');

        if (! $employeeId) {
            $employeeId = DB::table('employees')->insertGetId([
                'fullname' => 'HR Aratechnology',
                'email' => $employeeEmail,
        ...
        $this->command->info('HR Aratechnology created: email=hr@aratechnology.id password=Password123!');
    }
}
